...
async function build(dir, conf = null, reactProductionProfiling = false, debugOutput = false) {
    var _config$typescript, _namedExports$include, _namedExports;
    if (!(await (0, _isWriteable.isWriteable)(dir))) {
        throw new Error('> Build directory is not writeable. https://err.sh/vercel/next.js/build-dir-not-writeable');
    ...
    const {
        ...
        rewrites,
        redirects
    } = await (0, _loadCustomRoutes.default)(config);
    if (ciEnvironment.isCI && !ciEnvironment.hasNextSupport) {
        const cacheDir = _path.default.join(distDir, 'cache');
        const hasCache = await (0, _fileExists.fileExists)(cacheDir);
        if (!hasCache) { // Intentionally not piping to stderr in case people fail in CI when
            // stderr is detected.
            console.log(`${Log.prefixes.warn} No build cache found. Please configure build caching for faster rebuilds. Read more: https://err.sh/next.js/no-cache`);
        }
    }
    ...
    telemetry.record((0, _events.eventCliSession)(_constants2.PHASE_PRODUCTION_BUILD, dir, {
        cliCommand: 'build',
    ...
    const ignoreTypeScriptErrors = Boolean((_config$typescript = config.typescript) === null || _config$typescript === void 0 ? void 0 : _config$typescript.ignoreBuildErrors);
    await (0, _verifyTypeScriptSetup.verifyTypeScriptSetup)(dir, pagesDir, !ignoreTypeScriptErrors);
    let tracer = null;
    ...
    const pagePaths = await (0, _utils2.collectPages)(pagesDir, config.pageExtensions); // needed for static exporting since we want to replace with HTML
    // files
    ...
    const entrypoints = (0, _entries.createEntrypoints)(mappedPages, target, buildId, previewProps, config, loadedEnvFiles);
    ...
    if (numConflicting) {
        throw new Error(`Conflicting public and page file${numConflicting===1?' was':'s were'} found. https://err.sh/vercel/next.js/conflicting-public-file-page\n${conflictingPublicFiles.join('\n')}`);
    }
    ...
    const buildCustomRoute = (r, type) => {
        ...
        return {
            ...
            regex: (0, _loadCustomRoutes.normalizeRouteRegex)(routeRegex.source)
        };
    };
    ...
    if (clientConfig.optimization && (clientConfig.optimization.minimize !== true || clientConfig.optimization.minimizer && clientConfig.optimization.minimizer.length === 0)) {
        Log.warn(`Production code optimization has been disabled in your project. Read more: https://err.sh/vercel/next.js/minification-disabled`);
    }
    ...
    if (isLikeServerless) {
        const clientResult = await (0, _compiler.runCompiler)(clientConfig); // Fail build if clientResult contains errors
    ...
    if (result.errors.length > 0) { // Only keep the first error. Others are often indicative
        ...
        if (error.indexOf('private-next-pages') > -1 && error.indexOf('does not contain a default export') > -1) {
            const page_name_regex = /*#__PURE__*/ _wrapRegExp(/'private\x2Dnext\x2Dpages\/([\0-&\(-\uFFFF]*)'/, {
                page_name: 1
            });
        ...
        if (error.indexOf('private-next-pages') > -1 || error.indexOf('__next_polyfill__') > -1) {
            throw new Error('> webpack config.resolve.alias was incorrectly overridden. https://err.sh/vercel/next.js/invalid-resolve-alias');
        }
    ...
    hasNonStaticErrorPage = hasCustomErrorPage && (await (0, _utils2.hasCustomGetInitialProps)((0, _require.getPagePath)('/_error', distDir, isLikeServerless), runtimeEnvConfig, false));
    const analysisBegin = process.hrtime();
    await Promise.all(pageKeys.map(async page => {
        const actualPage = (0, _normalizePagePath.normalizePagePath)(page);
        const [selfSize, allSize] = await (0, _utils2.getJsPageSizeInKb)(actualPage, distDir, buildManifest, config.experimental.modern);
        let isSsg = false;
        let isStatic = false;
        ...
        if (nonReservedPage) {
            ...
            if (customAppGetInitialProps === undefined) {
                customAppGetInitialProps = (0, _utils2.hasCustomGetInitialProps)(isLikeServerless ? serverBundle : (0, _require.getPagePath)('/_app', distDir, isLikeServerless), runtimeEnvConfig, true);
                namedExports = (0, _utils2.getNamedExports)(isLikeServerless ? serverBundle : (0, _require.getPagePath)('/_app', distDir, isLikeServerless), runtimeEnvConfig);
            ...
            try {
                let workerResult = await staticCheckWorkers.isPageStatic(page, serverBundle, runtimeEnvConfig);
                ...
                } else if (workerResult.isStatic && customAppGetInitialProps === false) {
    ...
    if (serverPropsPages.size > 0 || ssgPages.size > 0) { // We update the routes manifest after the build with the
    ...
    if (staticPages.size > 0 || ssgPages.size > 0 || useStatic404) {
        ...
        const exportConfig = {
            ...config,
            initialPageRevalidationMap: {}, // Default map will be the collection of automatic statically exported
            ...
            exportPathMap: defaultMap => { // Dynamically routed pages should be prerendered to be used as
                ...
                ssgPages.forEach(page => {
                    if ((0, _utils.isDynamicRoute)(page)) {
                        ...
                        if (ssgStaticFallbackPages.has(page)) { // Override the rendering for the dynamic page to be treated as a
                            // fallback render.
                            defaultMap[page] = {
                                page,
                                query: {
                        ...
                        } else { // Remove dynamically routed pages from the default path map when
        ...
        const serverOutputDir = _path.default.join(distDir, isLikeServerless ? _constants2.SERVERLESS_DIRECTORY : _constants2.SERVER_DIRECTORY);
        const moveExportedPage = async (originPage, page, file, isSsg, ext) => {
            file = `${file}.${ext}`;
            ...
            const relativeDest = _path.default.relative(serverOutputDir, _path.default.join(_path.default.join(pagePath, // strip leading / and then recurse number of nested dirs
                ...
                originPage.substr(1).split('/').map(() => '..').join('/')), file)).replace(/\\/g, '/');
        ...
        for (const page of combinedPages) {
            ...
            const file = (0, _normalizePagePath.normalizePagePath)(page); // The dynamic version of SSG pages are only prerendered if the fallback
            ...
            if (isSsg) { // For a non-dynamic SSG page, we must copy its data file from export.
                if (!isDynamic) {
                ...
                } else { // For a dynamic SSG page, we did not copy its data exports and only
    ...
    telemetry.record((0, _events.eventBuildOptimize)(pagePaths, {
        ...
        hasReportWebVitals: (_namedExports$include = (_namedExports = namedExports) === null || _namedExports === void 0 ? void 0 : _namedExports.includes('reportWebVitals')) !== null && _namedExports$include !== void 0 ? _namedExports$include : false
    }));
    if (ssgPages.size > 0) {
        ...
        await _fs.promises.writeFile(_path.default.join(distDir, _constants2.PRERENDER_MANIFEST), JSON.stringify(prerenderManifest), 'utf8');
    ...
    await _fs.promises.writeFile(_path.default.join(distDir, _constants2.EXPORT_MARKER), JSON.stringify({
        version: 1,
        hasExportPathMap: typeof config.exportPathMap === 'function',
        exportTrailingSlash: config.trailingSlash === true
    }), 'utf8');
    await _fs.promises.unlink(_path.default.join(distDir, _constants2.EXPORT_DETAIL)).catch(err => {
        if (err.code === 'ENOENT') {
    ...
    await (0, _utils2.printTreeView)(Object.keys(mappedPages), allPageInfos, isLikeServerless, {
        distPath: distDir,
        buildId: buildId,
        pagesDir,
        useStatic404,
...
function generateClientSsgManifest(prerenderManifest, {
    ...
    const ssgPages = new Set([...Object.entries(prerenderManifest.routes) // Filter out dynamic routes
        .filter(([, {
            srcRoute
        }]) => srcRoute == null).map(([route]) => route), ...Object.keys(prerenderManifest.dynamicRoutes)
    ]);
    const clientSsgManifestPaths = ['_ssgManifest.js', isModern && '_ssgManifest.module.js'].filter(Boolean).map(f => _path.default.join(`${_constants2.CLIENT_STATIC_FILES_PATH}/${buildId}`, f));
    ...
    clientSsgManifestPaths.forEach(clientSsgManifestPath => (0, _fs.writeFileSync)(_path.default.join(distDir, clientSsgManifestPath), clientSsgManifestContent));
}
